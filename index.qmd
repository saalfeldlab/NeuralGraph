...
Across regimes, we observe qualitatively different modes of recoverability. In some cases, such as chaotic high-rank activity with sufficient data, connectivity recovery is robust and largely insensitive to hyperparameter choices. In others, notably low-rank regimes, recovery is strongly instance-dependent: some connectivities are reliably recovered, while others remain inaccessible despite extensive exploration, indicating basin-dependent identifiability. In contrast, for sparse or subcritical regimes, connectivity recovery exhibits sharp, regime-wide ceilings that persist across architectures, hyperparameters, and code-level modifications, pointing to intrinsic limits imposed by the information content of the observed activity.

Connectivity recovery succeeds for dense chaotic networks up to n=600 (100% convergence at 30k frames), low-gain networks (g=3, n=200) rescued by 30k frames, and g=2 partially rescued (42% at n=100/30k, 92% at n=200/30k with inverse lr_W). The dominant lever is n_frames: tripling from 10k to 30k transforms n=300 from 25% to 100% convergence, n=600 from 0% to 100%, and g=3/n=200 from 0% to 100%. Three structural limits identified: subcritical spectral radius (sparse 50%, conn~0.49), partial connectivity ceiling (fill<100%, conn ≈ filling_factor confirmed at 50/80/90/100%), and g=1 fixed-point collapse (eff_rank drops to 1 at 30k --- hardest regime). n=1000 mapped at 30k (conn=0.745, needs ~100k frames). ~90 principles established across 29 regimes over 348 iterations.
...
Can GNN recover connectivity from rank-20 dynamics (eff_rank ~12)? The landscape exploration found a breakthrough recipe (lr_W=3E-3, L1=1E-6 → conn_R^2^=0.993, test_R^2^=0.996) in 12 iterations. A dedicated 128-iteration exploration loop tested 14 random seeds and established a universal recipe template (lr=1E-4, edge_diff=10000, batch=8, n_epochs_init=2) with only lr_W, L1, and occasionally edge_diff requiring per-seed tuning. 11 of 14 seeds reach test_R^2^≥0.953 when optimally configured; coeff_edge_diff=20000 emerges as a lever for mid-tier seeds.

### [Sparse Connectivity](case-sparse.qmd)

Can code-level GNN modifications break the conn_R^2^=0.489 ceiling in the sparse regime? The dedicated 52-iteration exploration tested 10 code modifications across 3 random seeds. All failed: the ceiling is a **fundamental identifiability limit**, not an architectural degeneracy. The paradigm shift came from replacing lin_edge with fixed tanh --- identical conn_R^2^=0.489 but dynamics quality drops --- proving the MLP was improving predictions, not causing the W ceiling.
