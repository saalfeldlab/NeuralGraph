...
subtitle: "Recovering W from rank-20 neural dynamics --- 340 iterations of LLM-guided exploration across network scales"
---
...
## Scaling the Seed Survey (Blocks 12--22, 256 iterations at n=100)

The dedicated exploration was extended to **256 iterations across 70 random seeds**, providing statistically robust estimates of seed difficulty and learnability.

::: {.callout-tip}
## Key Result: 70-Seed Census

**70 seeds tested, 49 learnable (70.0%), 21 hard (30.0%).** The hard-seed rate of 30% is robust across the sample. All 21 hard seeds share the same V-recovery failure signature (V_R^2^ < 0.43). The standard recipe (lr_W=5E-3, L1=1E-5, 2ep) works for ~72% of learnable seeds, while the L1=1E-6 + 3ep rescue combo transforms ~54% of lower mid-tier seeds.
:::

### Seed Difficulty Distribution (n=100)

| Tier | Count | test_R^2^ range | Characteristics |
|------|:-----:|:---------------:|:----------------|
| Top-tier (≥0.990) | 16 | 0.991--0.999 | Standard recipe works or needs only L1=1E-6 |
| Upper mid-tier (0.960--0.989) | 15 | 0.960--0.987 | Often benefits from L1=1E-6 or n_epochs=3 |
| Lower mid-tier (0.900--0.959) | 13 | 0.900--0.958 | Needs L1=1E-6 + 3ep rescue combo |
| Fragile/Abandoned | 5 | 0.709--0.953 | Single narrow peak, untuneable |
| Hard (V-failure) | 21 | 0.244--0.597 | Irredeemable --- V_R^2^ stuck at ~0.38--0.43 |

### L1 Sensitivity Across 70 Seeds

L1=1E-6 was tested on 35 learnable seeds: **19 helped (56%), 10 hurt (29%), 6 neutral (15%)**. There is no reliable predictor of which seeds benefit --- it is not correlated with baseline test_R^2^, seed magnitude, or V_R^2^. The safe strategy is: try L1=1E-6, keep it if it helps, revert if it hurts.

---

## Scaling to n=200 (Blocks 23--26, 84 iterations)

The exploration shifted to **n=200 neurons** (still rank-20, n_frames=10,000) to test how network size affects recovery difficulty. 37 seeds were tested, enabling direct comparison with n=100.

::: {.callout-important}
## Critical Finding: n=200 Is Harder

**37 seeds tested at n=200: 22 learnable (59.5%), 15 hard (40.5%).** The hard-seed rate is significantly higher than n=100 (40.5% vs 30.0%). Multiple seeds that were top-tier at n=100 **flipped to hard** at n=200 (seeds 137, 1000, 6000, 17000, 20000, 21000). Connectivity recovery drops from conn_R^2^ ≈ 1.000 (n=100) to 0.916--0.989 (n=200).
:::

### n=100 → n=200 Seed Flips

Several seeds changed category dramatically when moving from n=100 to n=200:

| Seed | n=100 test_R^2^ | n=200 test_R^2^ | Direction |
|:----:|:---------------:|:---------------:|:---------:|
| 21000 | 0.999 (top-tier) | HARD | flipped ↓ |
| 20000 | 0.993 (top-tier) | HARD | flipped ↓ |
| 137 | 0.989 (good) | HARD | flipped ↓ |
| 1000 | 0.991 (rescued) | HARD | flipped ↓ |
| 7000 | 0.974 (mid-tier) | **0.998** (top-tier) | improved ↑ |
| 11000 | 0.977 (mid-tier) | **0.999** (top-tier) | improved ↑ |

The V-recovery failure at n=200 shows V_R^2^ as low as 0.07--0.21 (vs 0.38--0.43 at n=100), suggesting more severe failure modes at larger n.

### n=200 Established Principles

- **L1=1E-6 is more universally helpful** at n=200: helps seeds 42, 7, 99, 256, 500, 10000 (+0.002 to +0.012 in test_R^2^). But hurts seeds 314, 2000, 4000, 7000, 8000 --- still seed-dependent.
- **lr_W=6E-3 is catastrophic** at n=200 (was beneficial for some n=100 seeds). n=200 has a narrower lr_W window.
- **lr_W=5E-3 remains optimal** at n=200 --- the convergence boundary does not shift with n.

---

## N-Scaling Study (Blocks 24--29, 340 iterations total)

The final phase locked four parallel slots to n=200, n=400, n=600, and n=1000, with n_frames=100,000 and data_augmentation_loop=50, enabling a systematic study of how connectivity recovery scales with network size.

::: {.callout-tip}
## Key Result: W-Recovery Improves with n, but Dynamics Need Per-Scale Tuning

At n=1000, connectivity recovery is near-perfect (conn_R^2^=0.992) with excellent dynamics (test_R^2^=0.997) in a **single iteration**. At n=400/600, W-recovery is even better (conn_R^2^≈0.999) but dynamics degrade --- a "reverse pattern" solved by adjusting lr_W per scale. n=200 remains the hardest scale (conn_R^2^≈0.84--0.86).
:::

### N-Scaling Summary (Best per Scale)

| n_neurons | Best test_R^2^ | Best conn_R^2^ | Optimal lr_W | V_R^2^ | Notes |
|:---------:|:--------------:|:---------------:|:------------:|:------:|:------|
| 200 | 0.996 | 0.861 | 5E-3 | 0.859 | Partial W-recovery; hard-seed rate 40.5% |
| 400 | **0.990** | **0.999** | **6E-3** | 0.993 | Near-perfect W; lr_W=6E-3 confirmed across 2 seeds |
| 600 | 0.963 | 0.9995 | 4E-3 | 0.995 | Best W-recovery; lr_W is seed-dependent (4--5E-3) |
| 1000 | **0.997** | **0.992** | 5E-3 | 0.989 | Both metrics excellent in one iteration |

### The Reverse Pattern

At n=400 and n=600, several iterations show a striking "reverse pattern": **near-perfect connectivity recovery** (conn_R^2^ ≈ 0.999, V_R^2^ > 0.99) coexisting with **weak dynamics** (test_R^2^ = 0.87--0.92). This occurs because the GNN learns the correct W structure but the MLP has not co-adapted to produce accurate dynamics predictions.

The solution is **higher lr_W**: at n=400, lr_W=6E-3 transforms test_R^2^ from 0.917 to 0.978 (+0.061). The pattern suggests that **optimal lr_W scales with n**: n=200→5E-3, n=400→6E-3, n=600→4--5E-3, n=1000→5--7E-3.

### Is the n_neurons Dependency Understood?

**Partially.** The key findings:

1. **W-recovery improves monotonically with n** (conn_R^2^: 0.84 at n=200 → 0.999 at n=400/600). At larger n, the W matrix has more constraints from the data and the GNN is forced toward the true connectivity.

2. **Dynamics recovery is non-monotonic**: best at n=200 and n=1000, weaker at n=400/600. This is not a fundamental limit but a **hyperparameter mismatch** --- lr_W must be re-tuned per scale.

3. **Hard-seed rate varies with n**: 30% at n=100, 40.5% at n=200, present at n=400/600/1000 but not yet systematically quantified at larger scales. V-recovery failure persists at all scales.

4. **n=1000 is the sweet spot for low-rank recovery**: both dynamics and connectivity are excellent with the standard recipe, making it the easiest scale for this regime.
