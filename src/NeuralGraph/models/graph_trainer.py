...
def data_train_VAE(config, erase, best_model, device):
    """
    Minimal trainer for Calcium_Latent_Dynamics.
    Two losses:
      1) recon loss at frames n % 20 == 0:      x_n -> enc -> dec  (no latent update)
      2) predictive loss from n to n+20:        x_n -> enc -> 20 updates -> dec -> x_{n+20}
    """
    simulation_config = config.simulation
    train_config = config.training
    model_config = config.graph_model
    ...
    n_runs       = train_config.n_runs
    batch_size   = train_config.batch_size
    stride       = 20  # fixed per spec
    coeff_pred20 = getattr(train_config, "coeff_pred20", 1.0)  # weight for predictive loss

    # seed & logger (same style as data_train_flyvis)
    if train_config.seed != 42:
        torch.random.fork_rng(devices=device)
        torch.random.manual_seed(train_config.seed)
        np.random.seed(train_config.seed)

    log_dir, logger = create_log_dir(config, erase)  # same util used elsewhere
    logger.info("Training Calcium_Latent_Dynamics (Ca-only)")

    # ---- load dataset (matches repo patterns: try .pt then .npy) ----
    x_list = []
    run = 0
    ...
    x0         = x_list[0][0]                 # [N_neurons, F]
    n_neurons  = int(x0.shape[0])
    config.simulation.n_neurons = n_neurons   # keep config in sync like other trainers

    # model + optimizer (keep it simple; single Adam)
    model = Calcium_Latent_Dynamics(config=config, device=device)
    model.train()
    lr = train_config.learning_rate_start
    ...
    # resume (optional, same file naming convention)
    if best_model not in (None, "", "None"):
        net = f"{log_dir}/models/best_model_with_{n_runs - 1}_graphs_{best_model}.pt"
        if os.path.exists(net):
            state = torch.load(net, map_location=device)
            model.load_state_dict(state["model_state_dict"])
            try:
                optimizer.load_state_dict(state["optimizer_state_dict"])
            except Exception:
                pass
            start_epoch = int(str(best_model).split("_")[0])
            logger.info(f"Resumed from {best_model} (start_epoch={start_epoch})")
        else:
            start_epoch = 0
            logger.info(f"Checkpoint {best_model} not found; starting from scratch.")
    else:
        start_epoch = 0

    # ca channel index consistent with flyvis code: index 7 for calcium
    ca_idx = 7  # matches existing FlyVis indexing for Ca. :contentReference[oaicite:1]{index=1}

    # how many iterations per epoch? keep it proportional to data size; minimal & stable
    base_indices = np.arange(0, max(0, n_frames - stride), stride)
    iters_per_epoch = max(1, (len(base_indices) * n_runs) // max(1, batch_size))
    logger.info(f"iters/epoch: {iters_per_epoch}, batch_size: {batch_size}, stride: {stride}")

    # scalar for NaN replacement (same idea used elsewhere — replace NaNs with baseline) :contentReference[oaicite:2]{index=2}
    baseline_value = float(simulation_config.baseline_value)
    baseline_t = torch.tensor(baseline_value, dtype=torch.float32, device=device)
    ...
    for epoch in range(start_epoch, n_epochs + 1):
        ...
        for _ in range(iters_per_epoch):
            ...
            # sample runs/frames
            batch_runs = np.random.randint(0, n_runs, size=batch_size)
            ks = np.random.choice(base_indices, size=batch_size, replace=True)

            # Build batch tensors (B, N)
            X0_batch = []
            X20_batch = []
            valid_mask = []

            for rr, k in zip(batch_runs, ks):
                # Ca at n and n+20
                Ca = x_list[rr][:, :, ca_idx]  # shape: [T, N]
                # guard tensors on device & float
                x_n  = Ca[k].to(device=device, dtype=torch.float32)
                # fill NaNs with baseline
                x_n  = torch.where(torch.isnan(x_n), baseline_t, x_n)

                X0_batch.append(x_n[None, :])

                # will we use predictive loss? require k+20 < T
                if (k + stride) < Ca.shape[0]:
                    x_np20 = Ca[k + stride].to(device=device, dtype=torch.float32)
                    x_np20 = torch.where(torch.isnan(x_np20), baseline_t, x_np20)
                    X20_batch.append(x_np20[None, :])
                    valid_mask.append(True)
                else:
                    # placeholder (ignored later)
                    X20_batch.append(x_n[None, :])
                    valid_mask.append(False)

            X0 = torch.cat(X0_batch, dim=0)      # (B, N)
            X20 = torch.cat(X20_batch, dim=0)    # (B, N)
            valid_mask = torch.tensor(valid_mask, dtype=torch.bool, device=device)

            # ---- Loss 1: reconstruction (no latent update) ----
            y0_hat = model.forward_noupdate(X0)
            loss_recon = mse(y0_hat, X0)

            # ---- Loss 2: predictive n -> n+20 via 20 updates ----
            loss_pred = torch.tensor(0.0, device=device)
            if coeff_pred20 > 0 and valid_mask.any():
                # encode & do exactly 20 steps (don’t depend on model_config.n_update_steps here)
                enc = model.encoder(X0[valid_mask])                       # (Bv, 2Z)
                mu, logvar = enc.split(model.Z_dim, dim=-1)               # (Bv, Z), (Bv, Z)
                z = model.reparameterize(mu, logvar)                      # (Bv, Z)
                for _ in range(20):
                    z = z + model.update_net(z)                           # residual update
                y20_hat = model.decoder(z)                                # (Bv, N)
                loss_pred = mse(y20_hat, X20[valid_mask])

            loss = loss_recon + coeff_pred20 * loss_pred
            loss.backward()
            optimizer.step()

            total_loss += loss.item()

        # epoch end: logging + checkpoint (same naming convention)
        mean_loss = total_loss / max(1, iters_per_epoch)
        print(f"Epoch {epoch}. Loss: {mean_loss:.6f}")
        logger.info(f"Epoch {epoch}. Loss: {mean_loss:.6f}")
