...
def analyze_ising_model(x_list, delta_t, log_dir, logger, mc):
    ...
    #     print(f'loading existing sparse-J Ising analysis...')
    ...
    # else:
    #     print(f'computing sparse-J from approximated Ising analysis...')
    #     energy_stride = 1
    #     s, h, J, E = approximated_Ising(
    #         x=x_list[0],
    #         voltage_col=3,
    #         top_k=top_k,
    #         block_size=2000,
    ...
    if False:
        print('computing triplet approximate sparse-J KL analysis...')
        kl_results_approx = triplet_residuals_full(
            S_data_pm1=s,
            h_full=h,
            J_sparse=J,
            n_model=200_000,
        ...
        print("\n--- Triplet KL Results: Approximate (sparse-J) ---")
        for name, stats in kl_results_approx.items():
            print(f"triplet KL [{name}]: \033[38;5;208mmedian=\033[0m{stats['median']:.4f}, IQR=[{stats['q1']:.4f}, {stats['q3']:.4f}], n={stats['n']}")
            logger.info(f"triplet KL approx [{name}]: median={stats['median']:.4f}, IQR=[{stats['q1']:.4f}, {stats['q3']:.4f}], n={stats['n']}")

    if False:
        print('computing triplet Ising exact KL analysis...')
        _, kl_results_exact = triplet_residuals_from_sparseJ(
            S_pm1=s,
            J_sparse=J,
            n_per_stratum=1000,
        ...
        print("\n--- Triplet KL Results: Exact (local Ising) ---")
        for name, stats in kl_results_exact.items():
            # For exact, use 'iqr' key which is a list [q1, q3]
            q1, q3 = stats.get('iqr', [np.nan, np.nan])
...
def sample_stratified_triplets_from_sparse_J(J_sparse, n_per_stratum=1000, tau=0.0, rng_seed=0):
    ...
    max_trials = 200000
    trials = 0
    pbar = tqdm(total=n_per_stratum * len(buckets), desc="Sampling triplets", leave=False)
    prev_total = 0
    while any(len(buckets[k]) < n_per_stratum for k in buckets) and trials < max_trials:
        ...
        if m == 3 and len(buckets['triangle']) < n_per_stratum:
            ...
            added = True
        if added:
            total_now = sum(len(buckets[k]) for k in buckets)
            pbar.update(total_now - prev_total)
...
def triplet_residuals_from_sparseJ(
    ...
    strata, J_sym = sample_stratified_triplets_from_sparse_J(J_sparse, n_per_stratum=200, tau=0.0, rng_seed=0)
    ...
    for cat, triplets in strata.items():
        for (i, j, k) in tqdm(triplets, desc=f"processing {cat} triplets"):
            # --- build 10-neuron neighborhood anchored on (i,j,k)
            neigh_ranked = neighbors_by_absJ(J_sym, nodes=[i,j,k], k_each=k_neighbors_each, exclude=[i,j,k])
            ...
            p_data = empirical_triplet_marginal(S10, triplet_pos)               # (8,)
            p_mod  = model_triplet_marginal_from_exact(h10, J10, triplet_pos)   # (8,)
            # --- KL (nats)
            dkl = kl(p_data, p_mod, eps=1e-12, logbase=np.e)
