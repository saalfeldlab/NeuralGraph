...
The one-step derivative prediction is near-perfect (R^2^=0.9998, SSIM=0.9995). The residuals are negligible --- the GNN captures the instantaneous dynamics exactly.

![One-step derivative: near-perfect per-step accuracy (R^2^=0.9998, SSIM=0.9995)](assets/signal_iter_005/deriv_kinograph_montage.png){.lightbox group="iter005"}

### **Dynamics prediction: 10,000 steps rollout**

Despite perfect connectivity recovery and near-perfect one-step accuracy, the autonomous rollout diverges (R^2^=-0.938). This is **temporal phase drift** in chaotic dynamics: even with the correct W, the learned MLP does not replicate the true nonlinearity (tanh) exactly. Small per-step errors in the MLP output are amplified exponentially through the chaotic recurrent loop. The GNN visits the same attractor --- the kinograph patterns are qualitatively similar --- but the temporal alignment is lost. This is the same mechanism observed in the [low-rank case](case-low-rank.qmd), where rollout R^2^ degrades despite near-perfect W recovery.

![Rollout kinograph: GT vs GNN prediction over 10,000 frames --- rollout R^2^=-0.938](assets/signal_iter_005/kinograph_montage.png){.lightbox group="iter005"}

The per-neuron R^2^ distribution confirms the phase drift interpretation: mean R^2^ = 0.251 +/- 0.224, range [0.003, 0.777]. Some neurons track the GT well (high individual R^2^) while others are completely decorrelated --- consistent with the trajectory visiting the same regions of state space at different times.
